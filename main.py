...
def power_iteration(M: np.ndarray, e: float = -1.0) -> (np.ndarray, list):
    """
    ...
    if e < 0:
        e = 1e-10  # Default value, you can adjust if needed

    # Normalized random vector of proper size to initialize iteration
    v = np.random.rand(M.shape[0])
    v /= np.linalg.norm(v)

    # Initialize residual list and residual of the current eigenvector estimate
    residuals = []
    r = 2.0 * e
    ...
    while r > e:
        # Power iteration
        next_v = np.dot(M, v)
        eigenvalue = np.dot(next_v, v)
        next_v /= np.linalg.norm(next_v)

        # Compute residual
        r = np.linalg.norm(next_v - v)
        residuals.append(r)
...
def load_images(path: str, file_ending: str=".png") -> (list, int, int):
    ...
    filenames = lib.list_directory(path)
    filenames.sort()
    for filename in filenames:
        if filename.endswith(file_ending):
            img_p = path+filename
            img_d = mpl.image.imread(img_p)
            img_a = np.asarray(img_d, dtype=np.float64)
            images.append(img_a)

    # TODO set dimensions according to first image in images
    if images:
        dimension_y, dimension_x = images[0].shape[:2]
    else:
        dimension_y = 0
        dimension_x = 0
...
def setup_data_matrix(images: list) -> np.ndarray:
    ...
    D = np.zeros((len(images), images[0].size), dtype=np.float64)
...
def accumulated_energy(singular_values: np.ndarray, threshold: float = 0.8) -> int:
    ...
    normal_svals = singular_values / np.sum(singular_values)

    k = 0
    # TODO: Determine k that first k singular values make up threshold percent of magnitude
    accumulated_energy = np.cumsum(normal_svals)
    k = np.argmax(accumulated_energy >= threshold) + 1
...
def project_faces(pcs: np.ndarray, images: list, mean_data: np.ndarray) -> np.ndarray:
    ...
    for i, image in enumerate(images):
        normal_image = image.flatten() - mean_data
        coefficients[i, :] = np.dot(normal_image, pcs.T)
...
np.ndarray, list, np.ndarray):
    ...
    for i in range(coeffs_train.shape[0]):
        for j in range(coeffs_test.shape[0]):
            winkel = np.arccos(np.clip(np.dot(coeffs_train[i] / np.linalg.norm(coeffs_train[i]), coeffs_test[j] / np.linalg.norm(coeffs_test[j])), -1, 1))
            scores[i, j] = np.degrees(winkel)
